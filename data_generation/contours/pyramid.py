...
class Pyramid:
    ...
    def _get_endpoint(self):
        """
        錐の頂点がxy平面上でどこにくるかをランダムに求める
        """
        end_x = self.xy_size // 2 #random.randint(0, self.xy_size)
        end_y = self.xy_size // 2 #random.randint(0, self.xy_size)
        return (end_x,end_y)
    ...
    def _get_z_vertex(self,vertex,z):
        """
        Returns vertex of the slice at z
        """
        rate = z / self.D
        x = int(vertex[0] + (self.end_xy[0] - vertex[0]) * rate)
        y = int(vertex[1] + (self.end_xy[1] - vertex[1]) * rate)
    ...
    def _get_z_vertexes(self,z):
        ...
        """
        z_vertexes = np.array([self._get_z_vertex(v,z) for v in self.base_vertexes])
